...
while (true)
{
	if (device_state_change ())	//check if device state change has happened
		...
		if ((device_state () == onstate) &(rtc_switch_counter_value( ) >= 1 ))	//two conditions, condition 1 for long press and condition 2 for pressand release
			{
				if (nrf_gpio_pin_read (13) == 1 )	//end of long press on switch
					{
						if ( rtc_elapse_time() <= 0x32)	//check to determine if it is long press or press and release  ???reduce by half again
							{
				...
				//the main volum control loop
				if (device_state () == volume)
					{
						if ((nrf_gpio_pin_read (13) == 1) &( rtc_elapse_time() >= FIVESECONDS))	//no operation of switch, return back to on state
							{
								device_new_state(onstate);
								rtc_reset_time ();
							}
							else if ((nrf_gpio_pin_read (13) == 0) & ( rtc_elapse_time() >= HALFSECOND) )	//volume control is in increase volume state  ???you need timer to make this more perfect for volume increase
								{
									...
									{
										audiocodec_volume_control(0);
										rtc_reset_time ();
										rtc_switch_start_time();
									}
					}
					__SEV();		//Send Event is a hint instruction
					//    Wait For Event is a hint instruction that permits the processor to enter
					//	a low-power state until one of a number of events occurs
					__WFE();	//modify this to sleep and wake up for, might not be needed
					}
}
